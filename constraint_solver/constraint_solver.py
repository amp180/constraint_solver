...
# Function to map variable->domain mappings to a more restrictive set of variable->domain mappings.
DomainsConstraint = Callable[[
    DomainsType,
], DomainsType,]
# Function that partially checks if a possible solution is valid.
SolutionConstraint = Callable[[
    SolutionsType,
], bool,]
...
def _assert_domain(domain: Domain):
    ...
    assert (len(domain) == len(
        set(domain))), f"Values in domain should be unique. \n{ domain }"
    return None
...
def domain_get_value(domain: Domain) -> ValueType:
    ...
    _assert_domain(domain)
    assert domain_is_value(
        domain
    ), f"This can only be called on a domain that contains a single value, not { domain }."
    return domain[0]
...
def _check_solution_constraints(variables: SolutionsType,
                                constraints: List[SolutionConstraint]) -> bool:
...
def _domains_with_assigned_value(domains: DomainsType, var: str,
                                 value: ValueType):
    """Creates a copy of a variable->domain mapping with one domain replaced with a single value domain."""
    new_domains = dict(domains)
    new_domains[var] = [
        value,
    ]  # list (domain) with a single value.
    return new_domains


def _constrain_domains(domains: DomainsType,
                       constraints: List[DomainsConstraint]) -> DomainsType:
...
def solve(domains: DomainsType,
          domain_constraints: List[DomainsConstraint] = [],
          solution_constraints: List[SolutionConstraint] = [],
          *,
          _depth: int = 0,
          sorted_function=sorted) -> SolutionGenerator:
    ...
    if _depth == len(domains):
        ...
        values: SolutionsType = dict([(name, domain_get_value(domain))
        ...
        for value in current_domain:
            ...
            new_domains = _domains_with_assigned_value(domains, current_var,
                                                       value)
    ...
    for var, domain in domains.items():
        if domain_is_value(domain):
            for v, d in domains.items():
                if v != var:
                    domains[v] = list(
                        filter(lambda x: x != domain_get_value(domain), d))
...
def vars_not_equal_domain_constraint(var1: str, var2: str):
    ...
    def not_equal(domains: DomainsType) -> DomainsType:
        ...
        if domain_is_value(domains[var1]):
            new_domains[var2] = list(
                filter(lambda x: x != domain_get_value(domains[var1]),
                       domains[var2]))
        if domain_is_value(domains[var2]):
            new_domains[var1] = list(
                filter(lambda x: x != domain_get_value(domains[var2]),
                       domains[var1]))
        return new_domains
...
def vars_not_diagonal_on_grid_domain_constraint(x1: str, y1: str, x2: str,
                                                y2: str):
    ...
    def not_diagonal(domains: DomainsType) -> DomainsType:
        ...
        # once any three variables are set, narrow the possibilities on the 3rd.
        if domain_is_value(domains[x1]) and domain_is_value(domains[x2]):
            ...
            if domain_is_value(domains[y1]):
                ...
                new_domains[y2] = list(
                    filter(
                        lambda y2_value: abs(y1_value - y2_value) != abs(x1_value - x2_value),
                        domains[y2]))
            if domain_is_value(domains[y2]):
                y2_value = domain_get_value(domains[y2])
                new_domains[y1] = list(
                    filter(
                        lambda y1_value: abs(y1_value - y2_value) != abs(x1_value - x2_value),
                        domains[y1]))
        if domain_is_value(domains[y1]) and domain_is_value(domains[y2]):
            ...
            if domain_is_value(domains[x1]):
                ...
                new_domains[x2] = list(
                    filter(
                        lambda x2_value: abs(y1_value - y2_value) != abs(x1_value - x2_value),
                        domains[x2]))
            if domain_is_value(domains[x2]):
                x2_value = domain_get_value(domains[x2])
                new_domains[x1] = list(
                    filter(
                        lambda x1_value: abs(y1_value - y2_value) != abs(x1_value - x2_value),
                        domains[x1]))
        return new_domains
